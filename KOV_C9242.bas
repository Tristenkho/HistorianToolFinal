Attribute VB_Name = "KOV_C9242"
Option Explicit

'======================== SHEETS ========================
Private Const SH_DATA   As String = "Paste Data"
Private Const SH_LIMITS As String = "Product Limits"
Private Const SH_TAGMAP As String = "Tag Map"
Private Const SH_KOV    As String = "KOV"

'======================== PRODUCT =======================
Private Const PRODUCT_NAME As String = "Infineum C9242"

'======================== ROLES (from Tag Map) =========
Private Const ROLE_TT    As String = "TT"      ' Reactor temps
Private Const ROLE_PT    As String = "PT"      ' Reactor pressure
Private Const ROLE_MFT   As String = "MFT"     ' Maleic charge flow
Private Const ROLE_CFT   As String = "CFT"     ' Cooler / quench flow
Private Const ROLE_MTT   As String = "MTT"     ' Maleic line/storage temp

'======================== EDITABLE THRESHOLDS ==========
' --- Maleic charge band & holds
Private Const MFT_BAND_LO       As Double = 65
Private Const MFT_BAND_HI       As Double = 95
Private Const MFT_INBAND_HOLD   As Double = 10   ' min in-band to start
Private Const MFT_OUTBAND_HOLD  As Double = 10   ' min out-of-band to end
Private Const CHG_TRIM_IN_MIN   As Double = 10   ' trimming TT/MTT at charge start (min)
Private Const CHG_TRIM_OUT_MIN  As Double = 10   ' trimming before charge end (min)

' --- Soak (CFT-based end) ---
Private Const DEF_SOAK_CFT_MIN  As Double = 150
Private Const DEF_SOAK_HOLD_MIN As Double = 10

' --- Cool end: CFT <= 150 for 10 min ---
Private Const COOL_CFT_MAX      As Double = 150
Private Const COOL_HOLD_MIN     As Double = 10

' --- Strip (AFTER Soak): start when PT = 14.7 for = 10 min ; end when PT = 14.7 for = 10 min
Private Const STRIP_PT_ATM      As Double = 14.7
Private Const STRIP_HOLD_MIN    As Double = 10

'=======================================================
'                    PUBLIC ENTRY
'=======================================================
Public Sub KOV_Run_InfineumC9242_Main()
    Dim wb As Workbook: Set wb = ThisWorkbook
    Dim wsD As Worksheet, wsL As Worksheet, wsM As Worksheet, wsK As Worksheet

    On Error Resume Next
    Set wsD = wb.Worksheets(SH_DATA)
    Set wsL = wb.Worksheets(SH_LIMITS)
    Set wsM = wb.Worksheets(SH_TAGMAP)
    Set wsK = wb.Worksheets(SH_KOV)
    On Error GoTo 0

    If wsD Is Nothing Or wsL Is Nothing Or wsM Is Nothing Then
        MsgBox "Missing sheet(s). Need: Paste Data, Product Limits, Tag Map.", vbCritical
        Exit Sub
    End If
    If wsK Is Nothing Then
        Set wsK = wb.Worksheets.Add(After:=wsD)
        wsK.name = SH_KOV
    End If

    ' ---- headers / time ----
    Dim hdr As Object: Set hdr = BuildHeaderIndexAll(wsD)
    Dim cT As Long: cT = HeaderCol(hdr, "Time")
    If cT = 0 Then MsgBox "Missing 'Time' header in Paste Data.", vbCritical: Exit Sub

    Dim n As Long, t() As Double
    If Not BuildTimeVector(wsD, cT, t, n) Then
        MsgBox "Time column not recognized as date/time.", vbCritical
        Exit Sub
    End If

    ' ---- roles -> tags ----
    Dim roleTags As Object
    Set roleTags = GroupTagsByRole_Explicit(wsM, PRODUCT_NAME, hdr)

    ' ---- composites ----
    Dim TT() As Double, pt() As Double, MFT() As Double, cFT() As Double, MTT() As Double
    Dim nTT As Long, nPT As Long, nMFT As Long, nCFT As Long, nMTT As Long
    Dim dTT As Double, dPT As Double, dMFT As Double, dCFT As Double, dMTT As Double
    Dim vTT As Double, vPT As Double, vMFT As Double, vCFT As Double, vMTT As Double

    TT = CompositeMedian_AndStats(wsD, t, hdr, roleTags, ROLE_TT, n, nTT, dTT, vTT)
    pt = CompositeMedian_AndStats(wsD, t, hdr, roleTags, ROLE_PT, n, nPT, dPT, vPT)
    MFT = CompositeMedian_AndStats(wsD, t, hdr, roleTags, ROLE_MFT, n, nMFT, dMFT, vMFT)
    cFT = CompositeMedian_AndStats(wsD, t, hdr, roleTags, ROLE_CFT, n, nCFT, dCFT, vCFT)
    MTT = CompositeMedian_AndStats(wsD, t, hdr, roleTags, ROLE_MTT, n, nMTT, dMTT, vMTT)

    If Not SeriesExists(TT) Or Not SeriesExists(pt) Then
        MsgBox "Required roles missing (TT/PT) for '" & PRODUCT_NAME & "'.", vbCritical
        Exit Sub
    End If

    ' ---- output header ----
    wsK.Cells.ClearContents
    wsK.Range("A1:F1").value = Array("Product", "Role", "Tags used", "N", "Max", "StdDev")
    wsK.Range("A2").value = PRODUCT_NAME
    Dim rr As Long: rr = 2
    rr = PrintRoleSummary(wsK, rr, ROLE_TT, roleTags, nTT, dTT, vTT, PRODUCT_NAME)
    rr = PrintRoleSummary(wsK, rr, ROLE_PT, roleTags, nPT, dPT, vPT)
    rr = PrintRoleSummary(wsK, rr, ROLE_MFT, roleTags, nMFT, dMFT, vMFT)
    rr = PrintRoleSummary(wsK, rr, ROLE_CFT, roleTags, nCFT, dCFT, vCFT)
    rr = PrintRoleSummary(wsK, rr, ROLE_MTT, roleTags, nMTT, dMTT, vMTT)

    rr = rr + 2
    wsK.rows(rr - 1).RowHeight = 8
    wsK.Range("A" & rr & ":L" & rr).value = Array("Stage", "Start Time", "End Time", "Metric", "Value", "Min", "TV", "Max", "Result", "# from TV", "Label", "Notes")
    wsK.Range("A" & rr & ":L" & rr).Font.Bold = True
    rr = rr + 1

If Not SeriesExists(MFT) Then
    MsgBox "Maleic flow role (MFT) missing. Check Tag Map / Paste Data.", vbExclamation
    Exit Sub
End If

' -------------------- MALEIC CHARGE --------------------
If Not SeriesExists(MFT) Then
    MsgBox "Maleic flow role (MFT) missing. Check Tag Map / Paste Data.", vbExclamation
    Exit Sub
End If

Dim iM_Start As Long, iM_End As Long
iM_Start = FirstHold_InBand(MFT, MFT_BAND_LO, MFT_BAND_HI, t, MFT_INBAND_HOLD)
If iM_Start > 0 Then
    iM_End = FirstHold_OutOfBand(MFT, MFT_BAND_LO, MFT_BAND_HI, t, iM_Start + 1, MFT_OUTBAND_HOLD)
End If

If iM_Start > 0 And iM_End > iM_Start Then
    ' --- Charge metrics ---
    Dim ttCharge As Double, mttAvg As Double, rateAvg As Double
    ttCharge = TrimmedMeanTW(TT, t, iM_Start, iM_End, CHG_TRIM_IN_MIN, CHG_TRIM_OUT_MIN)
    If HasLimit(wsL, PRODUCT_NAME, "Maleic Charge", "Temperature") Then
        rr = WriteRow(wsK, rr, "Maleic Charge", t(iM_Start), t(iM_End), _
                      "Temperature (F)", Round(ttCharge, 1), wsL, PRODUCT_NAME, "Maleic Charge", "Temperature", False, _
                      "TW mean (trim " & CHG_TRIM_IN_MIN & "/" & CHG_TRIM_OUT_MIN & "m); window=MFT in [" & MFT_BAND_LO & "–" & MFT_BAND_HI & _
                      "] = " & MFT_INBAND_HOLD & "m; end when out-of-band = " & MFT_OUTBAND_HOLD & "m.")
    End If

    If SeriesExists(MTT) And HasLimit(wsL, PRODUCT_NAME, "Maleic Charge", "Charge Temperature") Then
        mttAvg = TrimmedMeanTW(MTT, t, iM_Start, iM_End, CHG_TRIM_IN_MIN, CHG_TRIM_OUT_MIN)
        rr = WriteRow(wsK, rr, "Maleic Charge", t(iM_Start), t(iM_End), _
                      "Charge Temperature (F)", Round(mttAvg, 1), wsL, PRODUCT_NAME, "Maleic Charge", "Charge Temperature", False, _
                      "MTT mean over Maleic window.")
    End If

    If HasLimit(wsL, PRODUCT_NAME, "Maleic Charge", "Rate") Then
        rateAvg = TrimmedMeanTW(MFT, t, iM_Start, iM_End, 0, 0)
        rr = WriteRow(wsK, rr, "Maleic Charge", t(iM_Start), t(iM_End), _
                      "Rate (lb/min)", Round(rateAvg, 1), wsL, PRODUCT_NAME, "Maleic Charge", "Rate", False, _
                      "MFT mean over Maleic window.")
    End If

    If HasLimit(wsL, PRODUCT_NAME, "Maleic Charge", "Time") Then
        Dim maleicH As Double: maleicH = HoursBetween(t(iM_Start), t(iM_End))
        rr = WriteRow(wsK, rr, "Maleic Charge", t(iM_Start), t(iM_End), _
                      "Time (h)", Round(maleicH, 2), wsL, PRODUCT_NAME, "Maleic Charge", "Time", True, _
                      "Hours from in-band =" & MFT_INBAND_HOLD & "m to out-of-band =" & MFT_OUTBAND_HOLD & "m.")
    End If

    ' -------------------- SOAK --------------------
    If SeriesExists(cFT) Then
        Dim iSoak_Start As Long, iSoak_End As Long
        iSoak_Start = iM_End
        iSoak_End = FirstHold_Single(cFT, ">=", DEF_SOAK_CFT_MIN, t, iSoak_Start + 1, DEF_SOAK_HOLD_MIN)

        If iSoak_End > iSoak_Start Then
            Dim soakT As Double, soakH As Double
            soakT = TrimmedMeanTW(TT, t, iSoak_Start, iSoak_End, 0, 0)
            soakH = HoursBetween(t(iSoak_Start), t(iSoak_End))
            If HasLimit(wsL, PRODUCT_NAME, "Soak", "Temperature") Then
                rr = WriteRow(wsK, rr, "Soak", t(iSoak_Start), t(iSoak_End), _
                              "Temperature (F)", Round(soakT, 1), wsL, PRODUCT_NAME, "Soak", "Temperature", False, _
                              "Start=Maleic end; End when CFT = " & DEF_SOAK_CFT_MIN & " for = " & DEF_SOAK_HOLD_MIN & "m (trim 10/10).")
            End If
            If HasLimit(wsL, PRODUCT_NAME, "Soak", "Time") Then
                rr = WriteRow(wsK, rr, "Soak", t(iSoak_Start), t(iSoak_End), _
                              "Time (h)", Round(soakH, 2), wsL, PRODUCT_NAME, "Soak", "Time", True, _
                              "Hours from Maleic end to CFT = " & DEF_SOAK_CFT_MIN & " held " & DEF_SOAK_HOLD_MIN & "m.")
            End If

            ' -------------------- COOL --------------------
            Dim iCool_Start As Long, iCool_End As Long
            iCool_Start = iSoak_End
            iCool_End = FirstHold_Single(cFT, "<=", COOL_CFT_MAX, t, iCool_Start + 1, COOL_HOLD_MIN)
            If iCool_End > iCool_Start Then
                Dim coolH As Double: coolH = HoursBetween(t(iCool_Start), t(iCool_End))
                If HasLimit(wsL, PRODUCT_NAME, "Cool", "Time") Then
                    rr = WriteRow(wsK, rr, "Cool", t(iCool_Start), t(iCool_End), _
                                  "Time (h)", Round(coolH, 2), wsL, PRODUCT_NAME, "Cool", "Time", True, _
                                  "Hours from Soak end to CFT = " & COOL_CFT_MAX & " held " & COOL_HOLD_MIN & "m.")
                End If
            End If

            ' -------------------- STRIP --------------------
            If SeriesExists(pt) Then
                Dim iStrip_Start As Long, iStrip_End As Long
                iStrip_Start = FirstHold_Single(pt, "<=", STRIP_PT_ATM, t, iSoak_End + 1, STRIP_HOLD_MIN)
                If iStrip_Start > 0 Then
                    iStrip_End = FirstHold_Single(pt, ">=", STRIP_PT_ATM, t, iStrip_Start + 1, STRIP_HOLD_MIN)
                End If
                If iStrip_Start > 0 And iStrip_End > iStrip_Start Then
                    Dim stripT As Double: stripT = TrimmedMeanTW(TT, t, iStrip_Start, iStrip_End, 0, 0)
                    If HasLimit(wsL, PRODUCT_NAME, "Strip", "Temperature") Then
                        rr = WriteRow(wsK, rr, "Strip", t(iStrip_Start), t(iStrip_End), _
                                      "Temperature (F)", Round(stripT, 1), wsL, PRODUCT_NAME, "Strip", "Temperature", False, _
                                      "Start: PT = " & STRIP_PT_ATM & " for = " & STRIP_HOLD_MIN & "m; End: PT = " & STRIP_PT_ATM & " for = " & STRIP_HOLD_MIN & "m (trim 10/10).")
                    End If
                    If HasLimit(wsL, PRODUCT_NAME, "Strip", "Time") Then
                        Dim stripH As Double: stripH = HoursBetween(t(iStrip_Start), t(iStrip_End))
                        rr = WriteRow(wsK, rr, "Strip", t(iStrip_Start), t(iStrip_End), _
                                      "Time (h)", Round(stripH, 2), wsL, PRODUCT_NAME, "Strip", "Time", True, _
                                      "Vacuum window between PT holds at " & STRIP_PT_ATM & " psia.")
                    End If
                End If
            End If
        End If
    End If
Else
    MsgBox "Maleic window not found (MFT band " & MFT_BAND_LO & "–" & MFT_BAND_HI & _
           ", in-hold " & MFT_INBAND_HOLD & " min).", vbExclamation
End If

    KOV_ColorizeAllTables wsK
    wsK.Columns("A:L").AutoFit
    KOV_Notify "KOV complete for '" & PRODUCT_NAME & "'."
End Sub

'=======================================================
'                HEADERS / TIME
'=======================================================
Private Function BuildHeaderIndexAll(ws As Worksheet) As Object
    Dim d As Object: Set d = CreateObject("Scripting.Dictionary")
    d.CompareMode = 1
    Dim lastCol As Long: lastCol = ws.Cells(1, ws.Columns.Count).End(xlToLeft).Column
    Dim c As Long, key$
    For c = 1 To lastCol
        key = Trim$(CStr(ws.Cells(1, c).Value2))
        If Len(key) > 0 Then d(key) = c
    Next c
    Set BuildHeaderIndexAll = d
End Function

Private Function HeaderCol(hdr As Object, key As String) As Long
    Dim k$: k = Trim$(key)
    If hdr.Exists(k) Then HeaderCol = hdr(k)
End Function

Private Function BuildTimeVector(ws As Worksheet, ByVal cTime As Long, ByRef t() As Double, ByRef n As Long) As Boolean
    Dim lastRow As Long: lastRow = ws.Cells(ws.rows.Count, cTime).End(xlUp).Row
    If lastRow < 3 Then Exit Function
    n = lastRow - 1
    ReDim t(1 To n)

    Dim i As Long, v As Variant
    For i = 1 To n
        v = ws.Cells(i + 1, cTime).value
        If IsDate(v) Then
            t(i) = CDbl(CDate(v))
        ElseIf IsNumeric(v) Then
            t(i) = CDbl(v)
        Else
            t(i) = 0
        End If
    Next i

    Dim tot As Double
    For i = 2 To n
        Dim dt As Double: dt = MinutesBetween(t(i - 1), t(i))
        If dt > 0 Then tot = tot + dt
    Next i
    BuildTimeVector = (tot > 0.5)
End Function

'=======================================================
'                TAG MAP (explicit roles)
'=======================================================
Private Function GroupTagsByRole_Explicit(wsMap As Worksheet, ByVal product As String, hdr As Object) As Object
    Dim d As Object: Set d = CreateObject("Scripting.Dictionary")
    d.CompareMode = vbTextCompare

    Set d(ROLE_TT) = New Collection
    Set d(ROLE_PT) = New Collection
    Set d(ROLE_MFT) = New Collection
    Set d(ROLE_MTT) = New Collection
    Set d(ROLE_CFT) = New Collection

    Dim lastRow As Long: lastRow = wsMap.Cells(wsMap.rows.Count, 1).End(xlUp).Row
    Dim r As Long, prod$, tag$, role$, tagHeader$

    For r = 2 To lastRow
        prod = Trim$(CStr(wsMap.Cells(r, 1).value))
        If Len(prod) = 0 Or StrComp(prod, product, vbTextCompare) <> 0 Then GoTo nxt

        tag = Trim$(CStr(wsMap.Cells(r, 2).value))
        role = UCase$(Trim$(CStr(wsMap.Cells(r, 3).value)))
        If Len(tag) = 0 Or Len(role) = 0 Then GoTo nxt

        If HeaderCol(hdr, tag) > 0 Then
            tagHeader = tag
        ElseIf HeaderCol(hdr, tag & ".Val") > 0 Then
            tagHeader = tag & ".Val"
        Else
            GoTo nxt
        End If

        If d.Exists(role) Then d(role).Add tagHeader
nxt:
    Next r

    Set GroupTagsByRole_Explicit = d
End Function

